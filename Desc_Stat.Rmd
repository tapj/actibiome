...
Les échantillons et les patients ne sont pas uniques On a 20 000 lignes environ
...
```{r,message=F,warning=F, fig.height=7, fig.width=10, warning=FALSE, r,message=F}
...
plot <- function(category, title, fill){
  ...
  p <- ggplot(map_sub, aes(fill=!!category, y=Freq, x=!!category), binaxis='y', stackdir='center') + 
...
p_age_category <- plot(age_category,'Age\nCategory', 'Age Category') + scale_x_discrete(limits=c("newborn","child","schoolage","adult","senior"))


#p_gender <- plot('gender','Patients Counts by Gender', 'Gender')

p_antibiotics_current_use <- plot(antibiotics_current_use,'Antibiotic\nintake', 'Antibiotics Current Use')

p_disease <- plot(disease,'Health\nstatus', 'Health status')
p_westernized <- plot(westernized,'Westernized\nlifestyle', 'Westernized lifestyle')

figure <- ggarrange(ggarrange(p_age_category,p_antibiotics_current_use, ncol = 2, widths = c(1,0.25),
                              #labels = c("Health","Antibiotic use")
                              labels = c("A","B")
                              ),
                    ggarrange(p_disease, p_westernized, ncol = 2, widths = c(1,0.5),
                              #labels = c("age category","westernized")
                              labels = c("C","D")
                              ),
                    #labels = c("A","B","C","D"),
                    nrow = 2)
...
ggsave("figures/figureS1_metadata_description.pdf")
ggsave("figures/figureS1_metadata_description.jpg")
...
all_table = map %>%
  select(`Antibiotic intake`=antibiotics_current_use,`Health status`=disease, `Age category` = age_category, `Westernized lifestyle`=westernized) %>%
  tbl_summary()


adult_only_table = map %>%
  filter(age_category == "adult") %>%
  select(`Antibiotic intake`=antibiotics_current_use,`Health status`=disease, `Age category` = age_category, `Westernized lifestyle`=westernized) %>%
  tbl_summary()

no_antibio_only_table = map %>%
  filter(antibiotics_current_use == "no") %>%
  select(`Antibiotic intake`=antibiotics_current_use,`Health status`=disease, `Age category` = age_category, `Westernized lifestyle`=westernized) %>%
  tbl_summary()

no_antibio_and_adult_only_table = map %>%
  filter(antibiotics_current_use == "no") %>%
  filter(age_category == "adult") %>%
  select(`Antibiotic intake`=antibiotics_current_use,`Health status`=disease, `Age category` = age_category, `Westernized lifestyle`=westernized) %>%
  tbl_summary()

tbl_merge(list(all_table,no_antibio_only_table, no_antibio_and_adult_only_table), tab_spanner = c("**All individuals**",  "**No antibiotic intake**", "**No antibiotic intake and adult only**")) %>% gtsummary::as_gt()

tbl_merge(list(all_table,no_antibio_only_table, no_antibio_and_adult_only_table), tab_spanner = c("**All individuals**",  "**No antibiotic intake**", "**No antibiotic intake and adult only**")) %>% as_hux_xlsx(file="table_recap.xlsx", include = everything(), bold_header_rows = TRUE)
...
p  <- ggboxplot(map_antibio, x="antibiotics_current_use", y="reads_bifid", fill="antibiotics_current_use",outlier.shape = NA )  + scale_y_log10()

p<-ggplot(map_antibio, aes(x=antibiotics_current_use, y=reads_bifid, fill=antibiotics_current_use)) +
...
p <- p + #scale_fill_brewer(palette="Dark2",labels = c("No (4921 adults)", "Yes (52 adults)")) +
  ...
  labs(x = NULL, y = NULL, title = "Antibiotic intake") #+ 
  #coord_cartesian(ylim=c(0,0.3))

p_antibio <- p + stat_compare_means() + theme(legend.position = "none")
...
p <- p +
  ...
  stat_compare_means( method = "wilcox", ref.group = "no", label.y = 0.8)

p_westernized <- p + theme(legend.position = "none")
...
p<-ggplot(map_adult_no_antibio, aes(x=disease, y=reads_bifid, fill=disease))+ geom_violin(outlier.shape = NA) + labs(fill= "Disease (adults count)") + scale_y_log10()
...
p <- p + scale_fill_brewer(palette="Dark2", labels = c("Control (4348)","Adenoma (29)", "Colorectal (74)", "Metabolic (320)", "Bowel (45)","Arthritis (85)", "BD (20)")) + 
  ...
  theme_minimal() + labs(x = NULL, y = NULL, title = title)+
  #coord_cartesian(ylim=c(0,0.9)) + 
  stat_compare_means(vjust = 20, label.x = "Adenoma") + 
  stat_compare_means(label = "p.signif", method = "wilcox", ref.group = "Control")
...
```{r}
p_disease$data %>%
  select(reads_bifid, disease) %>%
  group_by(disease) %>%
  summarise(median=median(reads_bifid), max=max(reads_bifid), min=min(reads_bifid), q25=quantile(reads_bifid,0.25), q75=quantile(reads_bifid,0.75))
...
p <- p + scale_fill_brewer(palette="Dark2", labels = c("newborn (137)","child (160)","schoolage (88)","Adult (4921)", "Senior (422)")) +  
  ...
  coord_cartesian() + 
  stat_compare_means(label.x ="child") +
  scale_x_discrete(limits=c("newborn","child","schoolage","adult","senior")) +
  stat_compare_means(label = "p.signif", method = "wilcox", ref.group = "newborn")
p
...
map_age %>% 
  select(reads_bifid, age_category) %>%
...
Plot which groups the 4 last ones

```{r,message=F,warning=F, fig.height = 8, fig.width = 10}

figure <- ggarrange( p_antibio, p_age, p_westernized, p_disease,
                    labels = c("A","B","C","D"),
                    ncol =2, widths = c(0.5,1),
                    nrow = 2)

annotate_figure(figure, top = text_grob("Bifidobacterium relative abundance", size = 18))
#figure

ggsave("figures/figureS2_bif_rel_abund_metadata.pdf")
ggsave("figures/figureS2_bif_rel_abund_metadata.jpg")
...
p<-ggplot(map_gender, aes(x=gender, y=reads_bifid, fill=gender))+ geom_violin(outlier.shape = NA) + labs(fill= "Gender") + scale_y_log10()
  
p <- p + scale_fill_brewer(palette="Dark2") + 
  theme_minimal() + 
  labs(x = NULL, y = NULL, title = "Relative Abundance of Bifidobacterium among Adults, no Antibiotics Takers")+
