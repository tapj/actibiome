...
merge(mean_rank, sd_rank, by=c("method","k")) %>%
  ...
  #geom_line(aes(x=k, y=mean_rank))
    ...
    geom_ribbon(aes(x=k, ymin = mean_rank - (1.96*(sd_rank/sqrt(5))), ymax = mean_rank + (1.96*(sd_rank/sqrt(5))),  group=method, fill =method), alpha = 0.2) +
  #geom_ribbon(aes(x=k, ymin = mean_rank - (1.96*(sd_rank/sqrt(30))), ymax = mean_rank + (1.96*(sd_rank/sqrt(30)))), alpha = 0.2) +
    ...
    scale_fill_brewer("",type="qual", label="BIC\n(mean [CI 95%])") +
    scale_color_brewer("",type="qual", label="BIC\n(mean [CI 95%])") +
    ylab("model optimum average rank") +
    theme(legend.position = c(0.8, 0.2)) -> criteria_dmm_plot
...
fit_genus_k1 <- mclapply.hack(1:1, dmn, count=count, verbose=FALSE, mc.cores = 4)
...
#no_bifid_ = data.frame(sample_id=sample_low_reads, bifidotype = rep(max(enterotype$bifidotype) +1,length(sample_low_reads)))
...
distribution$bifidotype <- as.character(distribution$bifidotype)

distribution$labels <- paste0("Partition ",as.character(distribution$bifidotype),"\n", c("ado_lon","ado_lon_bif","lon_pse","lon","ado_pse","lon_ado_cat","no_Bif"))

distr_clusters <- ggplot(distribution, aes(x = labels, y = Freq, fill= labels)) +  # Plot with values on top
  geom_bar(stat = "identity") +
  geom_text(aes(label = Freq, vjust=1.5),
            size=5,
            color="white") +
  labs( y = "Subject count", x = expression(paste(italic("Bifidobacterium"), " based gut microbiome partitions")) ) +
  theme_classic() +
  scale_fill_brewer("", type="qual", palette=2) +
  #scale_x_discrete(limits = as.character(1:max(distribution$bifidotype))) +
...
colnames(matrix_int) <- paste("Partition ",1:ncol(matrix_int))

colnames(matrix_int) <- paste0(paste("Partition ",1:ncol(matrix_int)), "\n", c("ado_lon","ado_lon_bif","lon_pse","lon","ado_pse","lon_ado_cat"))
...
my_palette <- viridis::viridis_pal()
...
write.csv(bif_DMM_alpha_weight_df$carpet, file="dmm_alpha_weight_log10.csv")
...
bif_DMM_alpha_weight_df$carpet %>%
  ...
  xlab(expression(paste(italic("Bifidobacterium"), "based gut microbiome partitions"))) + 
...
bif_DMM_alpha_weight_df$carpet %>%
  reshape2::melt() %>%
  mutate(value=ifelse(value< -5,NA,value)) %>%
  ggplot() + geom_tile(aes(x=Var1,y=Var2,fill=10^value)) +
  #geom_text(aes(x=Var1,y=Var2,label=round(10^value,4))) +
  scale_fill_viridis_c("DMM model\nalpha param.", trans="log10") +
  xlab(expression(paste(italic("Bifidobacterium"), " based gut microbiome partitions"))) + 
  ylab(expression(paste(italic("Bifidobacterium"), " species"))) +
  theme_classic() +
  theme(axis.text.y = element_text(face="italic")) -> bif_DMM_alpha_weight_plot
...
ggsave("figures/figure1_dmm_distributions_heatmap.pdf")
...
p_relative_abundance <- ggplot(sam_data, aes(x = bifidotype,
  ...
  scale_x_discrete(limits = as.character(1:max(sam_data$bifidotype)), 
                   labels = paste0("Partition ",as.character(distribution$bifidotype),"\n", c("ado_lon","ado_lon_bif","lon_pse","lon","ado_pse","lon_ado_cat","no_Bif"))) +
...
pairwise.wilcox.test(.$bif_rel, .$bifidotype, data=.) %>% 
  broom::tidy() %>%
  mutate(fdr=p.adjust(p.value, method="fdr")) %>%
  readr::write_tsv(file = "bif_rel_pairwise_wilcox_adult.tsv")


sam_data %>%
  filter(age_category=="adult") %>%
  group_by(bifidotype) %>%
  summarise(median=median(bif_rel), q25=quantile(bif_rel,0.25), q75=quantile(bif_rel,0.75)) %>%
    readr::write_tsv(file = "bif_rel_adult_median.tsv")
...
p_diversity2 <- ggplot(diversity, aes(x = bifidotype,
  ...
  xlab(expression(paste(italic("Bifidobacterium"), " based gut microbiome partitions"))) +
  scale_x_discrete(labels = paste0("Partition ",as.character(distribution$bifidotype),"\n", c("ado_lon","ado_lon_bif","lon_pse","lon","ado_pse","lon_ado_cat","no_Bif"))) +
  scale_fill_brewer(type="qual", palette = 2) +
  guides(fill=FALSE)
  #labs(x = "Bifidotype", y = "", title = "Shannon diversity",color= "") + 
  #coord_cartesian(ylim=c(0.8,4.5)) +
...
phy_diversity %>%
  rownames_to_column("sample_id") %>%
  merge(., enterotype, by="sample_id") %>%
  merge(sam_data, by="sample_id") %>%
  filter(age_category=="adult") %>%
pairwise.wilcox.test(.$Shannon, .$bifidotype.x, data=.) %>% 
  broom::tidy() %>%
  mutate(fdr=p.adjust(p.value, method="fdr")) %>%
  readr::write_tsv(file = "bif_shannon_pairwise_wilcox_adult.tsv")
...
phy_diversity %>%
  ...
  merge(sam_data, by="sample_id") %>%
  filter(age_category=="adult") %>%
  group_by(bifidotype.x) %>%
  summarise(median=median(Shannon), q25=quantile(Shannon,0.25), q75=quantile(Shannon,0.75))  %>%
    readr::write_tsv(file = "bif_shannon_adult_median.tsv")
...
figure <- ggarrange(p_relative_abundance, p_diversity,
                    labels = c("A","B"),
                    ncol =2,
                    nrow = 1)
figure
...
distribution <- function(category, type){
  ...
  if(category!="age_category") {
    ...
    labs(x = "Cluster K", y = "percent", fill = category) +
      scale_x_discrete(labels=paste0(paste("P",1:7, "\n", c("ado_lon","ado_lon_bif","lon_pse","lon","ado_pse","lon_ado_cat","No_Bif")))) +
...
distr_disease <- distribution('disease', "qual") +
  labs(x = expression(paste(italic("Bifidobacterium"), " based gut microbiome partitions")), y = "", fill= "disease") +
...
```{r}
distr_disease$data %>%
  mutate(category = ifelse(category=="Control","Control","unhealthy")) %>%
  group_by(category,bifidotype) %>%
  summarise(rows=sum(rows)) %>%
  reshape2::dcast(bifidotype~category, value.var = "rows") %>%
  tibble::column_to_rownames("bifidotype") %>%
  .[c(1,4),] %>% chisq.test()
...
distr_age <- distribution('age_category', "qual") +
  labs(x = expression(paste(italic("Bifidobacterium"), " based gut microbiome partitions")),fill= "age category") +
  coord_cartesian(ylim=c(0,1)) +  scale_fill_brewer("Age category", type="div", palette = 7)
...
distr_westernized <- distribution('westernized', "qual") +
  labs(x = expression(paste(italic("Bifidobacterium"), " based gut microbiome partitions")), fill= "westernized") +
...
(distr_age + scale_y_continuous("prevalence per partition", label=scales::percent, limits = c(0,1)) |
...
ggsave("figures/figure2_distributions.pdf")

```


```{r fig.height=5, fig.width=15}
bifidotype_vs_metadata_summary = rbind(
distr_age$data %>%
  reshape2::dcast(category~bifidotype, value.var = "rows", fill=0) %>%
  mutate(factor="age category"),

distr_westernized$data %>%
  reshape2::dcast(category~bifidotype, value.var = "rows", fill=0) %>%
   mutate(factor="westernized lifestyle"),

distr_disease$data %>%
  reshape2::dcast(category~bifidotype, value.var = "rows", fill=0) %>%
  mutate(factor="health status")

)
  
  colnames(bifidotype_vs_metadata_summary )= c("category",paste0(paste("P",1:7, "\n", c("ado_lon","ado_lon_bif","lon_pse","lon","ado_pse","lon_ado_cat","No_Bif"))),"factor")

  
  bifidotype_vs_metadata_summary %>% write.csv2(file="bifidotype_vs_metadata_summary.csv")

  
  bifidotype_vs_metadata_summary %>%
    reshape2::melt() %>%
    ggplot + geom_bar(aes(x=variable,y=value,fill=category), stat="identity") + 
    facet_grid(.~factor, scale="free") +
    coord_flip() +
    theme_classic() +
    labs(x = expression(paste(italic("Bifidobacterium"), " based gut microbiome partitions")), y ="number of individuals") +
    scale_fill_manual("",aesthetics = "fill", values = c(RColorBrewer::brewer.pal(5,"RdYlBu"),
                                                      RColorBrewer::brewer.pal(3,"Pastel1")[-3],
                                                      RColorBrewer::brewer.pal(7,"Accent")
                                                      ))
...
ggsave("figures/figureS4_partitions_bif_studies.pdf")
ggsave("figures/figureS4_partitions_bif_studies.jpg")
...
sampleMetadata %>% 
  ...
  select(bifidotype, number_reads) %>%
  group_by(bifidotype)%>%
  summarise(median=median(number_reads), q25 = quantile(number_reads, 0.25), q75 = quantile(number_reads, 0.75)) %>%
  readr::write_tsv(file = "bif_number_reads_desc.tsv")

```



## bifidotypes by countries


```{r}


merge(map,enterotype, by="sample_id") %>% 
  select(bifidotype, country, disease, westernized, age_category) %>% 
  tbl_summary(by=bifidotype) %>%
  as_hux_xlsx(file="bifidotypes_table_demo.xlsx")
